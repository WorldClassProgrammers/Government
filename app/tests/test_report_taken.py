...
class QueueReportApiTest(unittest.TestCase):
    ...
    def test_post_valid_report_taken(self):
        ...
        response = requests.post(url=endpoint, data={"citizen_id": 8888888888888, "vaccine_name": "Pfizer", "option": "reserve"})
    ...
    def test_post_report_taken_with_missing_attribute(self):
        ...
        endpoint = URL + "report_taken"
        response = requests.post(url=endpoint, data={"citizen_id": 8888888888888, "vaccine_name": "", "option": "reserve"})
    ...
    def test_post_report_taken_with_incorrect_vaccine(self):
        ...
        endpoint = URL + "report_taken"
        response = requests.post(url=endpoint, data={"citizen_id": 8888888888888, "vaccine_name": "Astra", "option": "reserve"})
        self.assertIn(b'vaccine_name not match reservation', response.content)

    def test_post_report_taken_with_invalid_vaccine(self):
        """Update vaccination info with wrong vaccine"""
        endpoint = URL + "report_taken"
        response = requests.post(url=endpoint, data={"citizen_id": 8888888888888, "vaccine_name": "Astro", "option": "reserve"})
    ...
    def test_post_report_taken_with_invalid_citizen_id(self):
        ...
        endpoint = URL + "report_taken"
        response = requests.post(url=endpoint, data={"citizen_id": 8899, "vaccine_name": "Pfizer", "option": "reserve"})
    ...
    def test_post_report_taken_with_citizen_id_not_yet_registered(self):
        ...
        endpoint = URL + "report_taken"
        response = requests.post(url=endpoint, data={"citizen_id": 8888888888799, "vaccine_name": "Pfizer", "option": "reserve"})
        self.assertIn(b'citizen ID is not registered', response.content)

    def test_get_report_taken_document(self):
        """Test get document for report taken api"""
        endpoint = URL + "document"
        response = requests.get(endpoint+"/report_taken")
        self.assertEqual(200, response.status_code)
        self.assertIn(b'To report vaccine taken', response.content)
